...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    ocr_language = str(data.get("capture", {}).get("ocr_language", "")) or derive_ocr_language(
        source_language
    )
    ...
    return AppConfig(
        ...
        opensubtitles_api_key=str(
            opensubtitles.get("api_key", legacy_opensubtitles.get("api_key", ""))
        ),
        opensubtitles_username=str(
            opensubtitles.get("username", legacy_opensubtitles.get("username", ""))
        ),
        opensubtitles_password=str(
            opensubtitles.get("password", legacy_opensubtitles.get("password", ""))
        ),
        opensubtitles_enable_org_fallback=_coerce_bool(
            opensubtitles.get(
                "enable_org_fallback", legacy_opensubtitles.get("enable_org_fallback", False)
            ),
            False,
        ),
        ...
        capture_interval_ms=_capture_interval_ms(
            data.get("capture", {}).get("interval_ms", DEFAULT_CAPTURE_INTERVAL_MS)
        ),
...
def config_from_payload(payload: dict[str, object], fallback: AppConfig | None = None) -> AppConfig:
    ...
    return AppConfig(
        opensubtitles_enabled=_coerce_bool(
            opensubtitles.get(
                "enabled", opensubtitles_legacy.get("enabled", base.opensubtitles_enabled)
            ),
            base.opensubtitles_enabled,
        ),
        opensubtitles_api_key=str(
            opensubtitles.get(
                "apiKey", opensubtitles_legacy.get("apiKey", base.opensubtitles_api_key)
            )
        ),
        opensubtitles_username=str(
            opensubtitles.get(
                "username", opensubtitles_legacy.get("username", base.opensubtitles_username)
            )
        ),
        opensubtitles_password=str(
            opensubtitles.get(
                "password", opensubtitles_legacy.get("password", base.opensubtitles_password)
            )
        ),
        opensubtitles_enable_org_fallback=_coerce_bool(
            opensubtitles.get(
                "enableOrgFallback",
                opensubtitles_legacy.get(
                    "enableOrgFallback", base.opensubtitles_enable_org_fallback
                ),
            ),
        ...
        tmdb_merge_enabled=_coerce_bool(
            tmdb.get("mergeEnabled", base.tmdb_merge_enabled), base.tmdb_merge_enabled
        ),
        ...
        target_language=normalize_target_language(
            str(languages.get("target", base.target_language))
        ),
        capture_region=_coerce_int_list(
            capture.get("region", base.capture_region), base.capture_region
        ),
        capture_interval_ms=_coerce_int(
            capture.get("intervalMs", base.capture_interval_ms), base.capture_interval_ms
        ),
        ...
        fuzzy_threshold=_coerce_int(
            matching.get("fuzzyThreshold", base.fuzzy_threshold), base.fuzzy_threshold
        ),
        match_window_size=_coerce_int(
            matching.get("windowSize", base.match_window_size), base.match_window_size
        ),
        ...
        overlay_font_size=_coerce_int(
            overlay.get("fontSize", base.overlay_font_size), base.overlay_font_size
        ),
        ...
        overlay_radius_px=_coerce_int(
            overlay.get("radiusPx", base.overlay_radius_px), base.overlay_radius_px
        ),
        overlay_padding_px=_coerce_int(
            overlay.get("paddingPx", base.overlay_padding_px), base.overlay_padding_px
        ),
        overlay_max_width_vw=_coerce_int(
            overlay.get("maxWidthVw", base.overlay_max_width_vw), base.overlay_max_width_vw
        ),
        overlay_blur_px=_coerce_int(
            overlay.get("blurPx", base.overlay_blur_px), base.overlay_blur_px
        ),
        overlay_shadow_strength=_coerce_float(
            overlay.get("shadowStrength", base.overlay_shadow_strength),
            base.overlay_shadow_strength,
        ),
        overlay_offset_pct=_coerce_int(
            overlay.get("offsetPct", base.overlay_offset_pct), base.overlay_offset_pct
        ),
        overlay_animation_ms=_coerce_int(
            overlay.get("animationMs", base.overlay_animation_ms), base.overlay_animation_ms
        ),
        debug_mode=_coerce_bool(
            payload.get("debug", {}).get("mode", base.debug_mode), base.debug_mode
        ),
    )
